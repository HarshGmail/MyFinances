...
export function registerMutualFundTools(server: McpServer, client: BackendClient): void {
  server.registerTool(
    ...
    {
      description: 'Fetch all mutual fund buy and sell transactions. Returns fund name, type (credit=buy/debit=sell), date, NAV, units, and amount for each transaction. If this tool fails or times out, retry it once.',
  ...
  server.registerTool(
    ...
    {
      description: 'Log a new mutual fund buy (credit) or sell (debit) transaction. If this tool fails or times out, retry it once.',
      inputSchema: z.object({
        ...
        nav: z.number().positive().describe('NAV (Net Asset Value) per unit at time of transaction in INR'),
    ...
    async (input) => {
      ...
      return {
        content: [{ type: 'text' as const, text: `Mutual fund transaction added successfully.\n${JSON.stringify(result, null, 2)}` }],
      };
    }
  );
}
